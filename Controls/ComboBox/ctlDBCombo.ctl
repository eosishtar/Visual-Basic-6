...
Begin VB.UserControl ctlDBCombo 
   ...
   ScaleHeight     =   3600
   ScaleWidth      =   4800
   ToolboxBitmap   =   "ctlDBCombo.ctx":0000
...
Public Function PopulateList(Conn As ADODB.Connection, tTable As String, tField As String, Blocked As Boolean, Optional vCatType As Integer, Optional Allocate As Boolean) As Boolean
...
Else
  'only for category
  strSql = "Select Distinct ID, CatType, Blocked, Deleted," & tField & " from " & tTable & " WHERE [CatType] = " & vCatType & " and [Blocked] = " & False & " AND [DELETED] = " & False & " ORDER BY " & tField
End If
